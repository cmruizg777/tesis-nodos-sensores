...
void callback(String topic, byte* message, unsigned int length) {
  ...
  Serial.println();
  // Enciende o apaga la lámpara, si el topic es level2/lamp
  if(topic=="level2/lamp"){
      int lampState = 0;
      ...
      client.publish("level2/actuators/lamp", messageActuators);
  }
  
  // Enciende o apaga los ventiladores, si el topic es level2/fan
  if(topic=="level2/fan"){
      ...
      client.publish("level2/actuators/fans", messageActuators);
  }
  // Enciende o apaga la electroválvula, si el topic es level2/valve
  if(topic=="level2/valve"){
      ...
      if(messageTemp == "on"){
        ...
        client.publish("level2/actuators/valve", messageActuators);  
        delay(2000);
        digitalWrite(valve, LOW);
        char message[3]="";
        client.publish("level2/autoOffValve", message);
      }
      else if(messageTemp == "off"){
        digitalWrite(valve, LOW);
        Serial.print("Off");
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Intentando conectar
    if (client.connect("ESP82661234")) {
      Serial.println("connected");  
      // Subscribe o resubscribe a un topic
      client.subscribe("level2/lamp");
      client.subscribe("level2/valve");
      client.subscribe("level2/fan");
    } else {
...
void loop() {
  ...
  if (now - lastMeasure > 1000) {
    ...
    client.publish("level2/sensors", message);
